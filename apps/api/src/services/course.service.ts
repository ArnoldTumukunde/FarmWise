...
export class CourseService {
    ...
    static async listCatalog(query: {
        ...
        if (duration) {
            ...
            const durationMap: Record<string, { gte?: number; lt?: number }> = {
                'under-1h': { lt: 3600 },
                'under-2h': { lt: 7200 },
        ...
        // Sort options
        let orderBy: Prisma.CourseOrderByWithRelationInput = { publishedAt: 'desc' };
        switch (sort) {
            case 'highest-rated': orderBy = { averageRating: 'desc' }; break;
            case 'most-enrolled': orderBy = { reviewCount: 'desc' }; break;
     ...
     */
    static async getRelatedCourses(slugOrId: string, limit = 8) {
