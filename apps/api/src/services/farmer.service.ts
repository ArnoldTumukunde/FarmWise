...
export class FarmerService {
   ...
   */
  static async getHomepageRecommendations(userId: string) {
   ...
   */
  static async getBecauseYouEnrolled(userId: string, courseId: string, limit = 8) {
   ...
   */
  static async getRecommendedTopics(userId: string, limit = 10) {
    const enrollments = await prisma.enrollment.findMany({
      where: { userId, status: 'ACTIVE' },
      include: {
        course: {
    ...
    for (const e of enrollments) {
      if (e.course.category?.name) topicSet.add(e.course.category.name);
      for (const t of e.course.tags) topicSet.add(t.tag.name);
    }
    ...
    if (topicSet.size < limit) {
      ...
      for (const c of cats) topicSet.add(c.name);
    }
  ...
  static async getStats(userId: string) {
    ...
    for (const enrollment of enrollments) {
      ...
      if (completed >= totalLectures) completedCount++;
    }
